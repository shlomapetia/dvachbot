...
HIRAGANA_MAP = {
    ...
    'дж': 'ぢ', 'дз': 'づ', 'джа': 'ぢゃ', 'дже': 'ぢぇ', 'джо': 'ぢょ', 'джу': 'ぢゅ', 'ж': 'ぢ', 'жа': 'ぢゃ', 'же': 'ぢぇ', 'жо': 'ぢょ', 'жу': 'ぢゅ',
...
KATAKANA_MAP = {
    ...
    'дж': 'ヂ', 'дз': 'ヅ', 'джа': 'ヂャ', 'дже': 'ヂェ', 'джо': 'ヂョ', 'джу': 'ヂュ',  'ж': 'ヂ', 'жа': 'ヂャ', 'же': 'ヂェ', 'жо': 'ヂョ', 'жу': 'ヂュ',
...
def anime_transform(text):
    ...
    # Упрощенное добавление японских элементов (без смайлов в тексте)
    for i in range(len(words)):
        ...
        # Иероглифы (2% вероятность) - только иероглиф без разъяснения
        if random.random() < 0.02 and KANJI_WORDS:
            kanji = random.choice(list(KANJI_WORDS.keys()))
            words.insert(i + 1, kanji)
            i += 1  # Пропуск добавленного слова
    
    # === ВОЗВРАЩЕННЫЕ СМАЙЛИКИ ===
    # Добавление смайликов в середину текста
    i = 0
    while i < len(words):
        if random.random() < 0.07:  # 7% шанс после слова
            words.insert(i + 1, random.choice(KAWAII_EMOJIS))
            i += 1  # Пропуск добавленного смайлика
        i += 1
    
    # Добавление смайлика в конец (40% шанс)
    ...
    # Добавляем конечную фразу (20% вероятность)
